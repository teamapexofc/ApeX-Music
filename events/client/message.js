...
module.exports = class MessageListener extends BaseEvent {
    ...
    async run(message) {
        ...
        if(message.content === `<@${this.client.user.id}>`) {
            ...
            .setDescription(`Hey, **${message.author.username}** <:WhiteLily:1352941837307875458>\nReady to use our bot **${this.client.user.username}** ?\n\n<:arrow:1352941872485629992>Use \`${prefix}Help\` To Know The Commands\n<:arrow:1352941872485629992>Use \`${prefix}Play <song name or url>\` To Play Music\n\n### More To Know\n\n<:arrow:1352941872485629992>Guild Prefix \`${prefix}\`\n<:arrow:1352941872485629992>Guild Count \`${this.client.guilds.cache.size}\``)
            .setImage('https://github.com/Alivelyyy/pvt.-resources/blob/main/standard%20(1).gif?raw=true');

            let row = new ActionRowBuilder().addComponents([
                new ButtonBuilder().setLabel("Invite Me").setStyle(ButtonStyle.Link).setURL(`https://discord.com/oauth2/authorize?client_id=${this.client.user.id}&permissions=8&integration_type=0&scope=bot+applications.commands`),
                new ButtonBuilder().setLabel("Support").setStyle(ButtonStyle.Link).setURL(`${this.client.config.serverLink}`),
                new ButtonBuilder().setLabel("Help").setStyle(ButtonStyle.Danger).setCustomId(`help-button`)
            ]);
