...
const { EmbedBuilder, ActionRowBuilder, ButtonBuilder, StringSelectMenuBuilder, WebhookClient, ButtonStyle } = require("discord.js");
    ...
    async run(interaction) {
        if (interaction.isButton()) {
            ...
            if (interaction.customId === "owner_del") {
                ...
                await player.data.get("nowPlayingMsg")?.edit({
                    components: [
                        new ActionRowBuilder().addComponents([
                            new ButtonBuilder().setLabel(state ? "Pause" : "Resume").setCustomId("pause-music").setStyle(ButtonStyle.Success),
                            new ButtonBuilder().setLabel("Previous").setCustomId("prevíous-music").setStyle(ButtonStyle.Secondary),
                            new ButtonBuilder().setLabel("Loop").setStyle(ButtonStyle.Primary).setCustomId("loop-music"),
                            new ButtonBuilder().setCustomId("skip-music").setLabel("Skip").setStyle(ButtonStyle.Secondary),
                            new ButtonBuilder().setCustomId(`stop-music`).setLabel("Stop").setStyle(ButtonStyle.Danger)
                        ])
                    ]
                }).catch((e) => { });
                ...
                let em1 = new EmbedBuilder().setColor(this.client.config.color).setDescription(`ApeX is a destined crystal quality audio parsing musical bot,\nfor all your freshing moods and entertainment.`).addFields([
                    {
                ...
                let embed = new EmbedBuilder().setColor(this.client.config.color).setDescription(`ApeX is a destined crystal quality audio parsing musical bot,\nfor all your freshing moods and entertainment.`).addFields([
                    {
                        name: 'Command Categories',
                ...
                let row2 = new ActionRowBuilder().addComponents([
                    new ButtonBuilder().setLabel("Home").setCustomId("home-help-menu").setStyle(ButtonStyle.Secondary).setDisabled(true),
                    new ButtonBuilder().setLabel("Command List").setCustomId("cmd-list-help").setStyle(ButtonStyle.Secondary),
                    new ButtonBuilder().setLabel("Button Menu").setCustomId("btn-menu-help").setStyle(ButtonStyle.Secondary)
                ]);
                ...
                let embed = new EmbedBuilder().setColor(this.client.config.color)
                    .setAuthor({
                        name: `${this.client.user.username}'s Help Menu`,
                        iconURL: this.client.user.displayAvatarURL({ forceStatic: false })
                    }).setDescription(`ApeX is a destined crystal quality audio parsing musical bot,\nfor all your freshing moods and entertainment.`)
                    .addFields([
                        {
                ...
                let btn1 = new ButtonBuilder().setEmoji(this.client.emoji.setup.previous).setStyle(ButtonStyle.Secondary).setCustomId("setup-previous");
                let btn2 = new ButtonBuilder().setEmoji(this.client.emoji.setup.stop).setStyle(ButtonStyle.Secondary).setCustomId("setup-stop");
                let btn3 = new ButtonBuilder().setEmoji(state ? this.client.emoji.setup.pause : this.this.client.emoji.setup.pause).setStyle(ButtonStyle.Secondary).setCustomId("setup-pause");
                let btn4 = new ButtonBuilder().setEmoji(this.client.emoji.setup.loop).setStyle(ButtonStyle.Secondary).setCustomId("setup-loop");
                let btn5 = new ButtonBuilder().setEmoji(this.client.emoji.setup.skip).setStyle(ButtonStyle.Secondary).setCustomId("setup-skip");

                let btn6 = new ButtonBuilder().setEmoji(this.client.emoji.setup.volLow).setStyle(ButtonStyle.Secondary).setCustomId("setup-volLow");
                let btn7 = new ButtonBuilder().setEmoji(this.client.emoji.setup.clearQueue).setStyle(ButtonStyle.Secondary).setCustomId("setup-clearQueue");
                let btn8 = new ButtonBuilder().setEmoji(this.client.emoji.setup.shuffle).setStyle(ButtonStyle.Secondary).setCustomId("setup-shuffle");
                let btn9 = new ButtonBuilder().setEmoji(this.client.emoji.setup.autoplay).setStyle(ButtonStyle.Secondary).setCustomId("setup-autoplay");
                let btn10 = new ButtonBuilder().setEmoji(this.client.emoji.setup.volHigh).setStyle(ButtonStyle.Secondary).setCustomId("setup-volHigh");
            ...
            for (let value of interaction.values) {
                if (value === "music-help") {
                    ...
                    let embed = new EmbedBuilder()
                        ...
                        .setTitle("ApeX Team Members")
                        .setDescription("The people behind ApeX bot development and maintenance.")
                        .addFields([
                            {
                                name: "Core Developers",
                                value: "The main developers who created and maintain ApeX."
                            },
                            {
                                name: "Contributors",
                                value: "Everyone who helped make ApeX better."
                            }
                        ])
                        .setFooter({
                            text: `Requested By: ${interaction.user.globalName ?? interaction.user.username}`,
